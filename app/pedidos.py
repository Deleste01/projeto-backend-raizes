...
from app.domain.models import Pedido, ItemPedido, Produto, Estoque, CANAIS_PEDIDO
from app.auth import login_required, roles_accepted
from app.serialization import normalizar_chaves

pedidos_bp = Blueprint('pedidos', __name__, url_prefix='/api')

STATUS_PRODUCAO = ('EM_PREPARACAO', 'PRONTO', 'ENTREGUE', 'CANCELADO')
...
def criar_pedido(current_user):
    ...
    req_data = normalizar_chaves(request.get_json() or {})

    unidade_id = req_data.get('unidade_id')
    itens_request = req_data.get('itens', [])
    forma_pagamento = req_data.get('forma_pagamento')
    canal_pedido = req_data.get('canal_pedido')
    ...
    if canal_pedido not in CANAIS_PEDIDO:
        return jsonify({
            'error': 'CANAL_INVALIDO',
            'message': f'Canal de pedido invalido. Aceitos: {list(CANAIS_PEDIDO)}'
        }), 422
...
def atualizar_status(current_user, pedido_id):
    ...
    if novo_status not in STATUS_PRODUCAO:
        return jsonify({'error': 'STATUS_INVALIDO', 'message': f'Status deve ser um dos seguintes: {list(STATUS_PRODUCAO)}'}), 400
